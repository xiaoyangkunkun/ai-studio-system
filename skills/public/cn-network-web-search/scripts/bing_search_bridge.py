#!/usr/bin/env python3
"""搜索桥接服务 — 模拟 SearXNG JSON API。主引擎 360 搜索(中文相关性好),兜底 cn.bing.com。
...
2026-08 实测:cn.bing.com 中文长尾查询相关性极差("福州餐厅推荐"返回城市百科/旅游攻略),
360 搜索(so.com)对同一查询返回高度相关结果 → 360 为主引擎,必应兜底。

零第三方依赖(python3 标准库),systemd 常驻 127.0.0.1:8899。
默认端口 8899:8888 常被宝塔面板占用,9119 是 Hermes dashboard。
"""
import json
import re
import urllib.parse
...
def so360_search(query: str, limit: int = 10) -> list:
    ...
    for m in re.finditer(r'<li class="res-list".*?</li>', html, re.S):
        ...
        desc = re.sub(r"<[^>]+>", "", pm.group(1)).strip() if pm else ""
        # so.com/link 跳转链接保留原样(Hermes 会用它抓取,提取时需跟随 302)
        results.append({
            "title": title,
            "url": u,
            "content": desc,
...
if __name__ == "__main__":
    ...
    print(f"Search bridge listening on {HOST}:{PORT} (360 primary, bing fallback)", flush=True)
