...
# 1. 隐藏关联发现
echo "--- 1. 隐藏关联发现 ---"
python3 ~/.hermes/scripts/hidden_connections.py 2>&1
echo ""

# 2. 知识健康度指标
echo "--- 2. 知识健康度指标 ---"
python3 ~/.hermes/scripts/health_report.py 2>&1
echo ""

# 3. Graph快照追踪
echo "--- 3. Graph快照追踪 ---"
python3 ~/.hermes/scripts/graph_snapshot.py 2>&1
echo ""

# 读取最新健康度评分
LATEST_HEALTH=$(cat ~/vault/wiki/analytics/health-report-latest.json 2>/dev/null)
...
# 读取隐藏关联数
LATEST_CONNECTIONS=$(cat ~/vault/wiki/analytics/hidden-connections-latest.json 2>/dev/null)
